 ...
 *  if the robot is using the shell adapter, logs and returns the name.
 ...
 *  if the robot is using the shell adapter, logs and returns.
 *  if the robot is using any other adapter, throws an error.
 */
function fetchConfigOrReportIssue(robot, configKey) {
  if (!process.env[configKey]) {
    let configErrorMessage = `Could not value for configKey: ${configKey}`
    if (robot.adapterName.toLowerCase() == "shell") {
      // this is local dev, just log it, return alertRoomName
      robot.logger.error(configErrorMessage)
    } else {
      // fail build if not using shell adapter: alerts won't work w/o valid room
