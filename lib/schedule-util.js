...
function syncSchedules(robot, storageKey, jobsInMemory) {
  ...
  for (id of Object.keys(nonCachedSchedules || {})) {
    // update brain jobs created with threadId instead of metadata
    job = maybeConvertJobFromBrain(nonCachedSchedules[id])
...
function formatJobForMessage(
  ...
  if (metadata && jobRoom && remindInThread) {
    ...
    if (jobFlow) {
      ...
      let encodedMessageOrThreadId = encodeURIComponent(
        metadata.message_id || metadata.thread_id,
      ).replace(/-/g, "%2D")

      let messageOrThreadUrl = ""
      if (metadata.thread_id) {
        messageOrThreadUrl = flowdock.URLs.thread
          .replace(/{flowPath}/, jobRoomPath)
          .replace(/{threadId}/, encodedMessageOrThreadId)
      } else {
        messageOrThreadUrl = flowdock.URLs.messageDetail
          .replace(/{flowPath}/, jobRoomPath)
          .replace(/{messageId}/, encodedMessageOrThreadId)
      }

      roomDisplayText = `(to [thread in ${jobRoomDisplayName}](${messageOrThreadUrl}))`
    }
  }
...
function logSerializedJobDetails(logger, serializedJob, messagePrefix, jobId) {
  let [pattern, user, , threadIdOrMetadata, remindInThread] = serializedJob
  logger.debug(
    `${messagePrefix} (${jobId}): pattern: ${pattern}, user: %o, message: (message redacted for privacy), threadIdOrMetadata: %o, remindInThread: ${remindInThread}`,
    user,
    threadIdOrMetadata,
  )
}

// TODO: remove this after sync has run on prod with this
// this is to ensure backwards-compatibility of jobs saved before
// metadata replaced threadId
function maybeConvertJobFromBrain(serializedJob) {
  let [, , , threadIdOrMetadata, remindInThread] = serializedJob

  if (typeof threadIdOrMetadata == "string") {
    serializedJob[3] = { thread_id: threadIdOrMetadata }
  }
...
class Job {
  constructor(
    ...
    this.metadata = metadata // TODO: strip to only needed metadata, or include all?
  ...
  start(robot) {
    return (this.job = scheduler.scheduleJob(this.pattern, () => {
      const envelope = {
        user: this.user,
        room: this.user.room,
        metadata: (this.remindInThread && this.metadata) || {}, // TODO: strip to only needed metadata, or include all?
      }
