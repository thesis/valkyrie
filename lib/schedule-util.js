...
function createScheduledJob(
  ...
  try {
    ...
    if (job) {
      logSerializedJobDetails(robot.logger, job, "New job created")
...
function updateScheduledJob(robot, jobsInMemory, storageKey, msg, id, message) {
  ...
  logSerializedJobDetails(robot.logger, serializedJob, "Updated job")
  formattedJob = formatJobForMessage(
    robot.adapter,
    job.pattern,
...
function cancelScheduledJob(robot, jobsInMemory, storageKey, msg, id) {
  ...
  logSerializedJobDetails(robot.logger, job.serialize(), "Cancelled job")
  formattedJob = formatJobForMessage(
    robot.adapter,
    job.pattern,
    isCronPattern(job.pattern),
...
function syncSchedules(robot, storageKey, jobsInMemory) {
  ...
  for (id of Object.keys(nonCachedSchedules || {})) {
    ...
    logSerializedJobDetails(robot.logger, job, "Synced job FROM brain")
  }
  ...
  return (() => {
    ...
    for (id of Object.keys(nonStoredSchedules || {})) {
      ...
      job.logSerializedDetails(robot.logger, job, "Synced job TO brain")
    }
    return result
  })()
}
...
function logSerializedJobDetails(logger, serializedJob, messagePrefix) {
  logger.info(
    `${messagePrefix}: [pattern, user, message, threadId, remindInThread] = [${
      serializedJob[0]
    }, %o, (message not logged, for privacy), ${serializedJob[3]}, ${
      serializedJob[4]
    }]`,
    serializedJob[1],
  )
