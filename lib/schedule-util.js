...
function createScheduledJob(robot, jobsInMemory, msg, room, pattern, message) {
  let id
...
function scheduleFromBrain(robot, storageKey, id, pattern, user, message) {
  const envelope = {
    user,
    room: user.room,
  }
...
function getAndFormatScheduledJobList(
  robotAdapter,
  jobsInMemory,
  showAll,
  rooms,
