    ...
    return (this.job = scheduler.scheduleJob(this.pattern, () => {
      ...
      if (!isCronPattern(this.pattern)) {
        ...
        if (CONFIG.dontReceive !== "1") {
          ...
          // Jobs created via `remind` retain thread_id, those from `schedule` don't, even if not recurring
...
function postMessageCallback(robot, runningJob, messageText) {
  return function(err, res, body) {
    ...
    if (CONFIG.dontReceive !== "1") {
      // Send message to the adapter, to allow hubot to process the message.
      let messageId =
        runningJob.metadata.message_id ||
        runningJob.metadata.thread_id ||
        undefined
      let messageObj = new TextMessage(runningJob.user, messageText, messageId)
