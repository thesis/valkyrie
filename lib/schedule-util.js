...
// TODO: handle inputString can not be converted to int
...
// There is no match (message contains no template string to replace)
// There is incomplete or no regex match when something close to template string syntax is passed (?).
// There is a match, but no corresponding function in the dispatcher.
// There is a match and corresponding function, but there's an error in function execution.
function processTemplateString(message) {
  ...
  let templateStringFormatted = TEMPLATE_STRING_DISPATCHER[
    templateStringCommand
  ](templateStringValue)
...
function formatJobForMessage(
  ...
  if (!!jobMessage.length) {
    try {
      ...
      throw new Error(
        `Problem processing message at job format for display: ${require("util").inspect(
          error,
        )}`,
      )
    }
