  ...
  start(robot) {
    return (this.job = scheduler.scheduleJob(this.pattern, () => {
      ...
      if (CONFIG.dontReceive !== "1") {
        ...
        let messageId =
          this.metadata.message_id || this.metadata.thread_id || undefined
        let messageObj = new TextMessage(this.user, processedMessage, messageId)
...
function postMessageCallback(robot, runningJob, processedScheduleMessage) {
  return function(err, res, body) {
    ...
    if (err) {
      ...
      robot.send(messageEnvelope, processedScheduleMessage)
