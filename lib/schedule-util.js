...
function startScheduledJob(
  ...
  const job = new Job(id, pattern, user, room, message, cb, threadId)
  job.start(robot)
  jobsInMemory[id] = job
...
function formatJobForMessage(
  ...
  if (jobThreadId && remindInThread && jobRoom) {
...
class Job {
  ...
  start(robot) {
    return (this.job = scheduler.scheduleJob(this.pattern, () => {
      const envelope = {
        user: this.user,
        room: this.user.room,
        metadata: { thread_id: (this.threadId && this.remindInThread) || null },
      }
  ...
  // will updating this cause index errors on unpacking?
  serialize() {
    return [
      this.pattern,
      this.user,
