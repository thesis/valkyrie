...
function processTemplateString(message) {
  ...
  try {
    ...
    templateStringFormatted = allowedCommand(templateStringValue)
  } catch (error) {
    throw new Error(
  ...
  start(robot) {
    return (this.job = scheduler.scheduleJob(this.pattern, () => {
      ...
      if (!isCronPattern(this.pattern)) {
        ...
        postMessageForRecurringJob(robot, this, processedMessage)
      }
...
function postMessageForRecurringJob(robot, job, processedScheduleMessage) {
  ...
  let postParams = {
    event: "message",
    content: processedScheduleMessage,
  }
  ...
  robot.adapter.bot.post(
    ...
    postParams,
    extraHeader,
    postMessageCallback(robot, job, processedScheduleMessage),
  )
}

function postMessageCallback(robot, runningJob, processedScheduleMessage) {
  return function(err, res, body) {
    ...
    if (err) {
      ...
      logSerializedJobDetails(
        robot.logger,
        serializedJob,
        "Updated job last url after posting latest occurrence",
        runningJob.id,
      )

      return robot.send(messageEnvelope, processedScheduleMessage)
    }

    if (res && res.flow && res.thread_id) {
      ...
      if (lastPostedFlow) {
        ...
        let encodedId = encodeThreadId(res.thread_id)
    ...
    // Update the job in brain (for any non-err case).
    serializedJob = updateJobInBrain(
      robot.brain,
      RECURRING_JOB_STORAGE_KEY,
      runningJob,
