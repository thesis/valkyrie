...
/**
 * Given a robot brain, and a job id from a scheduled job, returns:
 * The url of the posted message for the previous invocation of the job with
 * this id, or "(last url not found)" if the job is found but it does not have
 * a saved lastUrl value.
 * Throws an error if the job is not found in the robot brain.
 */
function lastUrl(inputString, robotBrain) {
  ...
  return serializedJob[3] && serializedJob[3].lastUrl
    ? serializedJob[3].lastUrl
    : "(last url not found)"
}
 ...
 */
function processTemplateString(message, robotBrain) {
  ...
  try {
    ...
    templateStringFormatted = allowedCommand(templateStringValue, robotBrain)
  } catch (error) {
    throw new Error(
...
class Job {
  ...
  start(robot) {
    return (this.job = scheduler.scheduleJob(this.pattern, () => {
      ...
      try {
        processedMessage = processTemplateString(this.message, robot.brain)
      } catch (error) {
        robot.logger.error(
