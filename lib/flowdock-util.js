...
function getRoomIdFromName(robotAdapter, roomName) {
  if (!robotAdapter.flows || !roomName) {
    return null
  }
...
/**
 * Given a robotAdapter, returns an array of ids for flows that the robot has
 * joined, based on Flowdock flow information.
 *
 ...
 */
function getAllJoinedFlowIds(robotAdapter) {
  if (!robotAdapter.flows) {
    return []
  }
...
/**
 * Given a robotAdapter, returns an array of ids for public (not invite-only)
 * flows that the robot has joined, based on Flowdock flow information.
 *
 * Returns an empty array if the robot has not joined any flows or if all
 * joined flows are invite-only. If the adapter isn't properly set up, or isn't
 * a flowdock adapter, returns an empty array.
 */
function getPublicJoinedFlowIds(robotAdapter) {
  if (!robotAdapter.flows) {
    return []
  }
  return robotAdapter.joinedFlows().map(flow => {
    if (flow.access_mode != "invitation") return flow.id
  })
}
...
function robotIsInRoom(robotAdapter, roomId) {
  return getAllJoinedFlowIds(robotAdapter).indexOf(roomId) >= 0
}
