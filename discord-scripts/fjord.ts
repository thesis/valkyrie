...
export default async function manageFjord(discordClient: Client, robot: Robot) {
  ...
  if (discordClient.application) {
    ...
    const existingCommands = await discordClient.application.commands.fetch()
    const formattedExistingCommands: ApplicationCommandData[] = Array.from(existingCommands.values()).map((command) => {
      const unknownCommand = command as unknown
      return unknownCommand as ApplicationCommandData
    })

    robot.logger.info(formattedExistingCommands)

    const fjordCommands: ApplicationCommandData[] = [
      {
        name: "debug",
    ...
    const allCommands = [...formattedExistingCommands, ...fjordCommands]

    // Wipe existing commands
    await discordClient.application.commands.set([])
    // add commands all together again
    await discordClient.application.commands.set(allCommands)
  }
