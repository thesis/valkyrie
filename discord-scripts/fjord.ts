...
export default function manageFjord(discordClient: Client, robot: Robot) {
  ...
  discordClient.on("messageCreate", async (message) => {
    ...
    if (message.content === "!fjord") {
      ...
      channel.send("Here's a list of Fjord commands. !issues <repo-owner> <repo-name> !stale-issues <repo-owner> <repo-name> !debug !activity <repo-owner> <repo-name>")
    }
    ...
    if (message.content === "!thread") {
      ...
      await handleThreadCreate(message, threadName, "thread test", "Setup a thread for you")
    }
        ...
        axios.get(`${webhookUrl}?${queryParams.toString()}`)
          .then(async (response) => {
            if (ChannelType.GuildPrivateThread) {
              ...
              await handleThreadCreate(message, threadName, "recent issues", response.data )
            }
          })
          .catch((error) => {
            channel.send(`Automation recent issues flow failed: ${error.message}`)
          })

      } else {
        channel.send("**Please provide these parameters: !issues <username> <repo> **");
      }
    }
    ...
    if (message.content.startsWith("!activity")) {
      ...
      if (parameters.length >= 2) {
        ...
        axios.get(`${webhookUrl}?${queryParams.toString()}`)
          .then(async (response) => {
            if (ChannelType.GuildPrivateThread) {
              ...
              await handleThreadCreate(message, threadName, "git activity", response.data )
            }
          })
          .catch((error) => {
        ...
        channel.send("**Please provide these parameters: !activity <username> <repo> **");
      }
    }
    ...
    if (message.content.startsWith("!exec")) {
      ...
      if (parameters.length >= 1) {
        ...
        channel.send("**Please provide these parameters: !exec <workflow-name>**");
      }
    }
