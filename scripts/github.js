            ...
            api.getOrganization(org).getTeams()
                .then((result) => {
                    if (result.status != 200) {
                        ...
                        let selectedTeams =
                            result.data.filter(_ => teamSlugs.indexOf(_.slug) > -1)

                        slugsById =
                            selectedTeams.reduce((slugsById, team) => {
                ...
                .then((teamResponses) => {
                    let { successes, failures } =
                        teamResponses.reduce(({ successes, failures }, res) => {
                            if (res.status == 200) {
                                successes.push(res)
                            } else {
                                failures.push(res)
                    ...
                    if (failures.length > 0) {
                        robot.logger.error(
                            `Got failures adding ${gitHubUsername} to ${teamSlugs.join(", ")} in ${org}: ` +
