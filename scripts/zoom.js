...
module.exports = function(robot) {
    zoom.getSession(process.env['ZOOM_API_KEY'], process.env['ZOOM_API_SECRET'])
        .then(session => ZOOM_SESSION = session)
        .catch(err => {
            robot.logger.error(
                "Failed to set up Zoom session:",
                util.inspect(err)
            );
        })
    ...
    robot.respond(/zoom/, (res) => {
        ...
        ZOOM_SESSION.nextAvailableMeeting()
            .then((meeting) => {
                res.send(`All set; open in [the app](${meeting.app_url}) or [your browser](${meeting.join_url})!`)
            })
            .catch((err) => {
                robot.logger.error(
